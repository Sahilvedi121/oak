class Marksheet 
        ...
        def stdntmark_calculation
          ...
          @student_subjmarks.each do |subj,markstype|  ;markstype.each{|type,marks| }
            ...
            print "percentage =#{@percentage.round(2)}"
            case @percentage
            when 90..100
              @grade="A+"
            when 75...90
    ...
    def print_marksheet
      ...
      @student_subjmarks.each{|subj,markstype| print "#{subj.to_s.rjust(10)}"; print "#{markstype[:theory_marks].to_i.to_s.rjust(18)}"
      print "#{markstype[:practical].to_i.to_s.rjust(20)}" "#{(markstype[:practical].to_i+markstype[:theory_marks].to_i).to_s.rjust(20)}"
      puts " D" if (markstype[:practical].to_i+markstype[:theory_marks].to_i) >= 75 }
